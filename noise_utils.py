...
def get_random_walk_noise_for_position_sequence(
    ...
    # change).
    position_sequence_noise = tf.concat([tf.concat([
        tf.zeros_like(velocity_sequence_noise[0:trainModel.NUM_CARGO_PARTICLES, 0:1]),
        tf.cumsum(velocity_sequence_noise[0:trainModel.NUM_CARGO_PARTICLES, :], axis=1)], axis=1), tf.zeros([trainModel.NUM_BOUNDARY_PARTICLES, trainModel.SEQUENCE_LENGTH + 1, 2], tf.float32)], axis=0)
