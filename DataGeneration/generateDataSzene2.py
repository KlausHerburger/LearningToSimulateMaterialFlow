"""Generates a dataset for a given configuration of Szene 2"""
...
filename = os.path.join(dirname, 'VirtuosModels\\Szene2.epf')

# Set folders to save Matlab/Numpy file
m = (open("Data/Matlab/Szene2" + '/' + FILENAME + '.mat', 'wb'))
n = (open("Data/Numpy/Szene2" + '/' + FILENAME + '.npy', 'wb'))
...
while continueLoop:
    ...
    if time != prevTime and time is not None:
        ...
        assert not any(e < 0 or e > 3 for e in currentYData)
        assert not time < 0
        ...
        if (itr > 3000) or ((pos1y > 2.5) and (pos2y > 2.5) and (pos3y > 2.5) and (pos4y > 2.5) and (pos5y > 2.5) and (pos6y > 2.5) and (pos7y > 2.5) and (pos8y > 2.5) and (pos9y > 2.5) ):
            continueLoop = False
